...
class Conv2D(nn.Module):
    ...
    stride : int
        Stride
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride, activation="elu", groupnormsize=4):
        super().__init__()
        self.kernel_size = kernel_size
        self.conv_base = nn.Conv2d(
            in_channels, out_channels, kernel_size=kernel_size, stride=stride)
        self.pad = nn.ConstantPad2d([kernel_size // 2] * 4, value=0)
        self.normalize = torch.nn.GroupNorm(groupnormsize, out_channels)
        if (activation == "elu"):
    ...
    def forward(self, x):
        ...
        # print("Initial Conv2D", x.shape)
        x = self.pad(x)
        # print("Initial Conv2D (pad)", x.shape)
        x = self.conv_base(x)
        # print("Middle Conv2D", x.shape)
        x = self.normalize(x)
        # print("Middle Conv2D (normalized)", x.shape)
        x = self.activ(x)
        # print("Final Conv2D", x.shape)
        return x
...
class PackLayerConv3d(nn.Module):
    ...
    into [B,(r^2)C,H/r,W/r] and then convolves it to produce [B,C,H/r,W/r].
    """
    def __init__(self, in_channels, kernel_size, r=2, d=8, activation="elu"):
        """
        ...
        self.conv = Conv2D(in_channels * (r ** 2) * d, in_channels, kernel_size, 1, activation)


    def forward(self, x):
        """Runs the PackLayerConv3d layer."""
        # print("1/6 PackLayer3DConv", x.shape)
        x = self.pack(x)
        # print("2/6(pack) PackLayer3DConv", x.shape)
        x = x.unsqueeze(1)
        # print("3/6(unsqueeze) PackLayer3DConv", x.shape)
        x = self.conv3d(x)
        # print("4/6(conv3d) PackLayer3DConv", x.shape)
        b, c, d, h, w = x.shape
        x = x.view(b, c * d, h, w)
        # print("5/6(view) PackLayer3DConv", x.shape)
        x = self.conv(x)
        # print("6/6(conv2d) PackLayer3DConv", x.shape)
        return x
...
class UnpackLayerConv3d(nn.Module):
    ...
    to produce [B,(r^2)C,H,W] and then unpacks it to produce [B,C,rH,rW].
    """
    def __init__(self, in_channels, out_channels, kernel_size, r=2, d=8, activation = "elu", groupnormsize=4):
        """
        ...
        """
        super().__init__()
        self.conv = Conv2D(in_channels, out_channels * (r ** 2) // d, kernel_size, 1, activation, groupnormsize)
        self.unpack = nn.PixelShuffle(r)
        self.conv3d = nn.Conv3d(1, d, kernel_size=(3, 3, 3),
                                stride=(1, 1, 1), padding=(1, 1, 1))
